...
class GenerationMixin:
    ...
    def _add_full_prompt_to_past(
        self, full_prompt: Tuple[Tuple[torch.Tensor]], model_kwargs: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def soft_greedy_search(
        ...
        **model_kwargs,
        ...
        cur_len = input_ids.shape[-1]
        sampled_next_tokens = (
            torch.nn.functional.one_hot(input_ids, num_classes=self.config.vocab_size)
            .float()
            .to(input_ids.device)
        )
        soft_tokens = (
        ...
        while True:
            ...
            if inputs_embeds is None:
                model_inputs["inputs_embeds"] = self.get_input_embeddings()(
            ...
            if not inference:
                ...
                cur_sampled_next_token_soft = torch.nn.functional.softmax(
            ...
            inputs_embeds = torch.matmul(
                cur_sampled_next_token, self.get_input_embeddings().weight
            )
    ...
    def soft_greedy_search_with_guider(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            guider_model_inputs = guider_model.prepare_inputs_for_generation(
                guider_input_ids, **guider_model_kwargs
            )
            ...
            if inputs_embeds is None:
                ...
                guider_model_inputs[
                    "inputs_embeds"
                ] = guider_model.get_input_embeddings()(
                    guider_model_inputs["input_ids"]
                )
            else:
                if len(inputs_embeds.size()) == 2:
                    model_inputs["inputs_embeds"] = inputs_embeds.unsqueeze(1)
                    guider_model_inputs["inputs_embeds"] = (
                        guider_inputs_embeds.unsqueeze(1)
                    )
                else:
                    model_inputs["inputs_embeds"] = inputs_embeds
            ...
            next_tokens_scores = logits_processor(
                input_ids, (next_token_logits + 0.2 * guider_token_logits) / 1.2
            )
            ...
            guider_inputs_embeds = torch.matmul(
                cur_sampled_next_token, guider_model.get_input_embeddings().weight
            )
    ...
    def soft_greedy_search_with_diff_mask(
        self,
        inputs_embeds: torch.Tensor = None,
        input_ids: Optional[torch.LongTensor] = None,
        logits_processor: Optional[LogitsProcessorList] = None,
        ...
        **model_kwargs,
        ...
        def masked_gumbell_softmax(logits, mask, bias_idx, tau=1):
            ...
            y_soft = (exp_gumbel * diff_mask[:, bias_idx, :]) / (
                exp_gumbel * diff_mask[:, bias_idx, :]
        ...
        def masked_softmax(logits, mask, bias_index):
            ...
            y_soft = (exp_logits * diff_mask[:, bias_idx, :]) / (
                exp_logits * diff_mask[:, bias_idx, :]
            ).sum(dim=-1, keepdim=True)
            next_tokens = torch.argmax(y_soft, dim=-1)
            y_hard = (
                torch.nn.functional.one_hot(
        ...
        while True:
            ...
            next_tokens_scores = (
                temp_logit.detach() + next_token_logits - next_token_logits.detach()
            )
            ...
            cur_sampled_next_token, next_tokens = masked_softmax(
                next_tokens_scores, diff_mask, bias_idx
            )
            ...
            cur_sampled_next_token_soft = (
                cur_sampled_next_token_soft * diff_mask[:, bias_idx, :]
            ) / (cur_sampled_next_token_soft * diff_mask[:, bias_idx, :]).sum(
                dim=-1, keepdim=True
            )
        ...
        **model_kwargs,
        ...
        while True:
            ...
            if use_scale_weights:
                ...
                biases_scaled_weighted = torch.nan_to_num(
                    biases[:, bias_idx, :] * scaling_ratio * weight, nan=0
                )
            else:
                scaling_ratio = 1
            ...
            if not use_hidden_states_biases:
                ...
                next_tokens_scores = next_tokens_scores + weight * self.lm_head(
                    biases[bias_idx]
                )
    ...
    def soft_greedy_search_with_biases_embed(
        self,
        inputs_embeds: torch.Tensor = None,
        input_ids: Optional[torch.LongTensor] = None,
        logits_processor: Optional[LogitsProcessorList] = None,
        ...
        **model_kwargs,
        ...
        while True:
            ...
            next_tokens_scores = (
                next_tokens_scores
                + weight * scaling_weight * self.lm_head(biases[:, bias_idx, :])
            )
