...
def construct_all_configs_comb(
    ...
    for exp in experiments:
        for ablation_k, ablation_v in ablation_conf.items():
            ...
            for ablation_val in ablation_v["values"]:
                ...
                if exp == "sentiment":
                    save_dir = initialize_metric_storing(
                        conf, f"{base_dir}/{conf['exp']}_pos/{sampler}"
                    )
                else:
                    save_dir = initialize_metric_storing(
                        conf, f"{base_dir}/{conf['exp']}/{sampler}"
                    )
                conf["prev_run_dir"] = save_dir
                configs_to_run.append((save_dir, conf))
    return configs_to_run
...
def worker_run_exp(run_dir, total_conf, gpu_queue, to_save_queue):
    ...
    try:
        if total_conf["exp"] == "detoxify":
            ...
            res = keywords_loop(
                total_conf, dump_sampling_metrics=False, return_sampling_metrics=True
            )
        elif total_conf["exp"] == "sentiment":
            res = sentiment_exp_loop(
                total_conf, dump_sampling_metrics=False, return_sampling_metrics=True
            )
        elif total_conf["exp"] == "selective_biasing":
            res = selective_biasing_loop(
                total_conf, dump_sampling_metrics=False, return_sampling_metrics=True
            )
        ...
        metrics = eval_loop(
            total_conf, generated_sentences, return_on_end=True, dump_on_end=False
        )
...
def run_ablations(all_conf, avail_gpus, jobs_per_gpu):
    ...
    while True:
        ...
        if not jobs_queue.empty() and not gpu_queue.empty():
            ...
            pool.apply_async(
                worker_run_exp,
                args=(run_dir, total_conf, gpu_queue, to_save_queue),
            )
            time.sleep(0.1)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--do_all_combs", action="store_true"
