...
def conditional_perplexity(
    ...
    for i, row in tqdm(
        ...
        prompt_input_ids = tokenizer.encode(row.prompt["text"], return_tensors="pt").to(
            device
        )
        if not (
...
def allsat_accuracy(generations_df):
    ...
    for i, row in tqdm(
        generations_df.iterrows(),
        total=len(generations_df.index),
        desc="Scoring generation allsats",
    ):
...
def sentiment_classify(generations_df, sentiment_file=None):
    ...
    for i, row in tqdm(
        generations_df.iterrows(),
        total=len(generations_df.index),
        desc="Scoring generation sentiments",
    ):
        prompt = row.prompt["text"]
        generations = [gen["text"] for gen in row["generations"]]
        ...
        try:
            ...
            predictions_for_prompt = [{"label": "", "score": float("nan")}] * len(
                sentences_for_prompt
            )
...
def sentiment_classify_own(generations_df, sentiment_file=None):
    ...
    classifier_model = GPT2CustomForSequenceClassification.from_pretrained(
...
def sentiment_classify_own2(generations_df, sentiment_file=None):
    ...
    config = AutoConfig.from_pretrained(model_path)
    tokenizer = AutoTokenizer.from_pretrained(model_path)
    classifier_model = RobertaCustomForSequenceClassification.from_pretrained(
        model_path, config=config
    )
    classifier = TextClassificationPipeline(
        task="text-classification", model=classifier_model, tokenizer=tokenizer
    )
    print("writing outputs to ", str(sentiment_file))
    if sentiment_file is not None:
        fo = open(sentiment_file, "w")
...
def politeness_classify(generations_df, politeness_file=None):
    ...
    for i, row in tqdm(
        generations_df.iterrows(),
        total=len(generations_df.index),
        desc="Scoring generation politeness",
    ):
        prompt = row.prompt["text"]
        generations = [gen["text"] for gen in row["generations"]]
        sentences_for_prompt = []
...
def distinctness2(
    generations_df,
):  # not over samples but averaged over individual outputs
    dist1, dist2, dist3 = [], [], []
    # calculate dist1, dist2, dist3 across generations for every prompt
    for i, row in tqdm(
        generations_df.iterrows(),
...
@click.option(
    "--output_file", required=True, type=str, help="filename to write outputs"
)
@click.option(
    "--metrics",
    required=True,
    type=str,
    help="which metrics to compute, write comma separeted, ppl-own,ppl-big,cola,self-bleu,zipf,repetition,dist-n,sentiment",
)
@click.option(
    "--extra",
    required=False,
    type=str,
    help="extra params like which topic category or keyword file",
)
def main(generations_file, output_file, metrics, extra):
    ...
    if generations_file.endswith(".jsonl"):
        ...
        with open(generations_file) as fin:
            generations_df = [
                {"prompt": {"text": ""}, "generations": [{"text": l.strip()}]}
    ...
    if "ppl-big" in metricset:  # GPT2-XL
        ...
        with torch.no_grad():
            ppl, total_ppl = conditional_perplexity(
                ...
                eval_tokenizer,
                device=device,
                write_file=output_dir / (output_file + ".ppl-big"),
            )

        # write output results
        with open(output_dir / output_file, "a") as fo:
            fo.write(
                f"gpt2-xl perplexity, gpt2-xl total perplexity = {ppl}, {total_ppl}\n"
            )
            print(
                f"gpt2-xl perplexity, gpt2-xl total perplexity = {ppl}, {total_ppl}\n"
            )
    ...
    if "ppl-own" in metricset:  # GPT2-Large
        ...
        with torch.no_grad():
            ppl, total_ppl = conditional_perplexity(
                ...
                eval_tokenizer,
                device=device,
                write_file=output_dir / (output_file + ".ppl-own"),
            )

        # write output results
        with open(output_dir / output_file, "a") as fo:
            fo.write(
                f"gpt2-large perplexity, gpt2-large total perplexity = {ppl}, {total_ppl}\n"
            )
            print(
                f"gpt2-large perplexity, gpt2-large total perplexity = {ppl}, {total_ppl}\n"
            )
    ...
    if "sentiment" in metricset:
        ...
        sentiment_accuracy, sentiment_std = sentiment_classify_big(
            generations_df, sentiment_file=output_dir / (output_file + ".sentiment-big")
        )
        with open(output_dir / output_file, "a") as fo:
            fo.write(
                f"mean sentiment-big accuracy = {sentiment_accuracy}, {sentiment_std}\n"
            )
            print(
                f"mean sentiment-big accuracy = {sentiment_accuracy}, {sentiment_std}"
            )
        ...
        print("sentiment")  # c1
        sentiment_accuracy, sentiment_std = sentiment_classify(
            generations_df, sentiment_file=output_dir / (output_file + ".sentiment")
        )
        with open(output_dir / output_file, "a") as fo:
            fo.write(
                f"mean sentiment accuracy = {sentiment_accuracy}, {sentiment_std}\n"
            )
            print(f"mean sentiment accuracy = {sentiment_accuracy}, {sentiment_std}")

        print("sentiment-yelp")  # c2
        sentiment_accuracy, sentiment_std = sentiment_classify_yelp(
            generations_df,
            sentiment_file=output_dir / (output_file + ".sentiment-yelp"),
        )
        with open(output_dir / output_file, "a") as fo:
            fo.write(
                f"mean sentiment-yelp accuracy = {sentiment_accuracy}, {sentiment_std}\n"
            )
            print(
                f"mean sentiment-yelp accuracy = {sentiment_accuracy}, {sentiment_std}"
            )
        ...
        sentiment_accuracy, sentiment_std = sentiment_classify_own2(
            generations_df, sentiment_file=output_dir / (output_file + ".sentiment-own")
        )
        with open(output_dir / output_file, "a") as fo:
            fo.write(
                f"mean sentiment-own accuracy = {sentiment_accuracy}, {sentiment_std}\n"
            )
            print(
                f"mean sentiment-own accuracy = {sentiment_accuracy}, {sentiment_std}"
            )
    ...
    if "sentiment_promptonly" in metricset:
        print("sentiment")
        sentiment_accuracy, sentiment_std, num_neutral = sentiment_classify_promptonly(
            generations_df, sentiment_file=output_dir / (output_file + ".sentiment")
        )
        with open(output_dir / output_file, "a") as fo:
            fo.write(
                f"mean sentiment accuracy = {sentiment_accuracy}, {sentiment_std}, {num_neutral}\n"
            )
            print(
                f"mean sentiment accuracy = {sentiment_accuracy}, {sentiment_std}, {num_neutral}"
            )
    ...
    if "toxicity" in metricset:
        print("toxicity")
        (avg_max, toxic_probability) = toxicity_score(
            generations_df, perspective_file=output_dir / (output_file + ".toxicity")
        )
        with open(output_dir / output_file, "a") as fo:
    ...
    if "allsat" in metricset:
        ...
        with open(output_dir / output_file, "a") as fo:
            fo.write(
                f"mean allsat accuracy, std = {sat_accuracy}--{sat_std}, {sat_once}\n"
            )
    ...
    if "politeness" in metricset:
        print("politeness")
        polit_accuracy, politeness_std, politeness_once = politeness_classify(
            generations_df, politeness_file=output_dir / (output_file + ".politeness")
        )
        with open(output_dir / output_file, "a") as fo:
            fo.write(
                f"mean politeness accuracy, std = {polit_accuracy}--{politeness_std}, {politeness_once}\n"
            )
            print(
                f"mean politeness accuracy, std = {polit_accuracy}--{politeness_std}, {politeness_once}"
            )
    ...
    if "topic" in metricset:
        print(f"topic -- {extra}")
        num_match, num_unit_match, total_sent_count = topic_eval(
            generations_df, extra, None
        )

        with open(output_dir / output_file, "a") as fo:
            fo.write(
                f"num match, num unit match, total sent count: {num_match}--{num_unit_match}, {total_sent_count}\n"
            )
            print(
                f"num match, num unit match, total sent count: {num_match}--{num_unit_match}, {total_sent_count}"
            )
