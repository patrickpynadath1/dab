...
class UniqueNessLoss(BaseLoss):
    ...
    def __init__(self, model, tokenizer, args):
        ...
        self.C = (
            (
                torch.tril(torch.ones((args.max_output_length, args.max_output_length)))
                - torch.diag(torch.ones(args.max_output_length))
            )
            .to(self.device)
            .detach()
        )
    ...
    def compute_loss(self, batch, preds, **kwargs):
        ...
        input_logprobs = torch.log_softmax(
            -torch.cdist(input_embeds, embed_lut.weight).square(), dim=-1
        )
        ...
        unll_q = (torch.exp(unll) * C.unsqueeze(0)) / (
            (torch.exp(unll) * C.unsqueeze(0)).sum(dim=-1, keepdims=True) + 1e-8
        )
    ...
    def compute_gold_loss(self, batch, **kwargs):
        ...
        with torch.no_grad():
            ...
            unll = torch.log(ul)  # * C.unsqueeze(0)
            unll_q = (ul * C.unsqueeze(0)) / (
                (ul * C.unsqueeze(0)).sum(dim=-1, keepdims=True) + 1e-8
            )
