...
class KeywordL2Loss(BaseLoss):
            ...
            try:
                logprobs = (
                    F.conv2d(
                        ...
                        self.eye_filter[:, :, :ngram_length, :ngram_length],
                    )
                    / ngram_length
                )
                ...
                if step % self.gumbel_update_step == 0:
                    self.logprobs_q = F.gumbel_softmax(
                        logprobs / self.tau, hard=True, dim=-1
                    )  # .detach()
    ...
    def process_keyword(self, keyword):
        if self.keyword == keyword:
            ...
            for (
                ...
                all_combos
            ):  # all combos is meaningless at the moment, there is only one combo
                if self.args.target_tokenize_different:
                    with self.tokenizer.as_target_tokenizer():
                        self.keywords_ids.append(
                ...
                ndist = F.log_softmax(
                    ndist, dim=-1
                )  # .index_select(dim=-1, index=self.keywords_ids[-1])
                # print(ndist.size())
                ...
                self.epsilon = max(
                    self.epsilon, -ndist.max(dim=-1)[0].mean(dim=0).item() + delta
                )
    ...
    def compute_gold_loss(self, batch, **kwargs):
        ...
        with torch.no_grad():
            ...
            embed_lut = self.model.get_input_embeddings()
            ndist = -torch.square(
                torch.cdist(embed_lut.weight.unsqueeze(0), pred_embeds)
            )

            losses = []
            for kid, keyword_id in enumerate(self.keywords_ids):
                logprobs = F.log_softmax(ndist, dim=1).index_select(
                    dim=1, index=keyword_id
                )
                ngram_length = keyword_id.size(0)
                try:
                    logprobs = (
            ...
            # print(losses)
            losses = torch.stack(losses, dim=1)
            losses_q = F.gumbel_softmax(
                -losses / self.tau, hard=True, dim=-1
            )  # .detach()
