...
class GPT2ConditionalLoss(BaseLoss):
    ...
    def compute_loss(self, batch, preds, **kwargs):
        ...
        source = kwargs.get(
            "additional_batch"
        )  # in STRAP model, real_source x is paraphrased to source y which is then transformed into target y. The language model sees only source and target, not the real source
        ...
        pad = (
            torch.empty((batch_size, pad_length))
            .long()
            .to(self.device)
        ...
        source_segment_id = (
            torch.empty((batch_size, pad_length + source.size(1)))
            .long()
            .to(self.device)
            .fill_(self.tokenizer.additional_special_tokens_ids[1])
        )
        target_segment_id = (
            torch.empty((batch_size, prefix.size(1) + pred_tokens.size(1) + 2))
            .long()
            .to(self.device)
            .fill_(self.tokenizer.additional_special_tokens_ids[2])
        )
        ...
        if losstype == "xentropy":
            model_output = self.model(
                inputs_embeds=input_embeds, token_type_ids=segment
            )
            ...
            if prefix.size(1) > 0:
                xentropy_prefix = (
                    F.nll_loss(
                        lm_logprobs[:, : prefix.size(1), :].squeeze(0),
                        prefix.squeeze(0),
                        reduction="none",
                    )
                    .unsqueeze(0)
            ...
            xentropy_pred = (
                (-lm_logprobs[:, prefix.size(1) : -2, :] * pred_probs)
                .sum(dim=-1)
                .sum(dim=-1)
            )  # / (pred_probs.size(-1))
            xentropy_pred = (
                xentropy_pred
                - lm_logprobs[:, -2, self.eos_token_id]
                - lm_logprobs[:, -2, self.eos_token_id]
            )
            ...
            if losstype == "cosine":
                ...
                # input()
                hidden_states_unitnorm = torch.nn.functional.normalize(
                    hidden_states, p=2, dim=-1
                ).contiguous()
                pred_embs_unitnorm = torch.nn.functional.normalize(
                    input_embeds[:, source.size(1) + pad_length + 1 :, :], p=2, dim=-1
                ).contiguous()
                loss = (
                    1.0 - (hidden_states_unitnorm * pred_embs_unitnorm).sum(dim=-1)
                ).sum(dim=-1)

            elif losstype == "dot":
                hidden_states = hidden_states.contiguous()
                pred_embs = input_embeds[
                    :, source.size(1) + pad_length + 1 :, :
                ].contiguous()
                ...
                loss += torch.logsumexp(
                    hidden_states.matmul(embed_lut.weight.t()), dim=-1
                ).detach()
                # loss += torch.log(torch.exp(hidden_states.matmul(embed_lut.weight.t())).sum(dim=-1))
                loss = loss.sum(dim=-1)
                # loss = (-hidden_states * pred_embs).sum(dim=-1).sum(dim=-1)
    ...
    def compute_gold_loss(self, batch, **kwargs):
        ...
        target_segment_id = (
            torch.empty((batch_size, target.size(1) + 2))
            .long()
            .to(self.device)
            .fill_(self.tokenizer.additional_special_tokens_ids[2])
        )
        ...
        if losstype == "xentropy":
            ...
            loss = (
                F.nll_loss(
                    lm_logprobs[:, : target.size(1) - 1, :].squeeze(0),
                    target[:, 1 : target.size(1)].squeeze(0),
                    reduction="none",
                )
                .unsqueeze(0)
                .sum(dim=-1)
            ...
            if losstype == "cosine":
                ...
                pred_embs_unitnorm = torch.nn.functional.normalize(
                    input_embeds[:, source.size(1) + pad_length :, :], p=2, dim=-1
                )[:, 1:, :].contiguous()
                loss = (
                    1.0 - (hidden_states_unitnorm * pred_embs_unitnorm).sum(dim=-1)
                ).sum(dim=-1)
                ...
                loss = -(hidden_states * pred_embs).sum(dim=-1)
                loss += torch.logsumexp(
                    hidden_states.matmul(self.model.get_input_embeddings().weight.t()),
                    dim=-1,
                )
                # loss += torch.log(torch.exp(hidden_states.matmul(self.model.get_input_embeddings().weight.t()).sum(dim=-1)))
                loss = loss.sum(dim=-1)
    ...
    def _prepare_input_for_generation(self, input_ids, **kwargs):
        ...
        max_prefix_length = getattr(
            self.args, "max_prefix_length", input_ids.size(1) + 1
        )
        ...
        bos = (
            torch.empty((input_ids.size(0), 1))
            .long()
            .to(self.device)
            .fill_(self.bos_token_id)
        )
        pad = (
            torch.empty((input_ids.size(0), pad_length))
            .long()
            .to(self.device)
        ...
        source_segment_id = (
            torch.empty((batch_size, source_segment_length))
            .long()
            .to(self.device)
            .fill_(self.tokenizer.additional_special_tokens_ids[1])
        )
