...
class BARTLoss(BaseLoss):
    ...
    def compute_loss(self, batch, preds, **kwargs):
        ...
        if losstype == "xentropy":
            model_output = self.model(
                input_ids=source,
                decoder_inputs_embeds=scaled_target_input_embeds[:, :-1],
            )
            ...
            xentropy_pred = (
                -lm_logprobs[:, prefix.size(1) + 1 : -1, :] * pred_probs
            ).sum(
                dim=-1
            )  # + 1 because of extra eos in the front
            ...
            final_logits_biases = self.model.final_logits_bias[
                0, target_input_tokens[:, 1:]
            ]  # needs to be a part of the trainable parameters maybe?
            ...
            if losstype == "dot":
                ...
                loss = (
                    -(hidden_states.detach() * pred_embs).sum(dim=-1)
                    - final_logits_biases
                )

                logits = hidden_states.matmul(
                    self.model.get_decoder().get_input_embeddings().weight.t()
                )
                ...
                logits = (
                    hidden_states.matmul(embed_lut.weight.t())
                    + self.model.final_logits_bias
                ) / temperature
                maxlogit = logits.max(dim=-1, keepdim=True)[0]
                logits = logits - maxlogit
                additive = torch.exp(
                    ((hidden_states * pred_embs).sum(dim=-1) + final_logits_biases)
                    / temperature
                    - maxlogit.squeeze(-1)
                ) - torch.exp(
                    (
                        (hidden_states * pred_embs.detach()).sum(dim=-1)
                        + final_logits_biases
                    )
                    / temperature
                    - maxlogit.squeeze(-1)
    ...
    def compute_gold_loss(self, batch, **kwargs):
        ...
        if losstype == "xentropy":
            model_output = self.model(
                input_ids=source,
                decoder_input_ids=target_input_tokens[:, :-1],
                labels=target_input_tokens[:, 1:],
            )
            ...
            loss = F.nll_loss(
                lm_logprobs.squeeze(0),
                target_input_tokens[:, 1:].squeeze(0),
                reduction="none",
            )
            ...
            input_embeds = self.model.get_decoder().get_input_embeddings()(
