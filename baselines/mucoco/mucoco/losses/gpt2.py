...
class GPT2Loss(BaseLoss):
    ...
    def get_coeff(self, step, seq_len, sched="constant"):
        if step == 0:
            ...
            # increasing.reverse()
            self.total = (
                torch.Tensor(constant + decreasing).unsqueeze(0).to(self.device)
            )
    ...
    def compute_loss(self, batch, preds, **kwargs):
        ...
        """
        prompt, prefix = (
            batch  # prompt is the real deal, prefix can be provided as an extended prompt (generated by the model autoregressively)
        )
        ...
        input_tokens = torch.cat(
            [self.extra_prefix, prompt, prefix, pred_tokens, context], dim=1
        )
        input_embeds = torch.cat(
            [
                embed_lut(self.extra_prefix),
                embed_lut(prompt),
                embed_lut(prefix),
                pred_embeds,
                embed_lut(context),
        ...
        if losstype == "xentropy":  # TODO
            ...
            model_output = self.model(
                inputs_embeds=input_embeds, step=step, go_inside="transformer"
            ...
            if losstype == "cosine":
                ...
                coeff = self.get_coeff(
                    step, hidden_states.size(1), sched=self.coeff_schedule
                )
                ...
                additive = (
                    ...
                    - torch.exp(
                        (hidden_states * pred_embs).sum(dim=-1) - maxlogit.squeeze(-1)
                    ).detach()
                )
                # print(additive)
                ...
                C[bandwidth:, : L - bandwidth] = C[
                    bandwidth:, : L - bandwidth
                ] - torch.tril(torch.ones(L - bandwidth, L - bandwidth)).to(self.device)
                C = C.detach()
                ...
                # print("what", (hidden_states * pred_embs).sum(dim=-1))
                additive = torch.exp(
                    (hidden_states * pred_embs).sum(dim=-1) / temperature
                    - maxlogit.squeeze(-1)
                ) - torch.exp(
                    (hidden_states * pred_embs.detach()).sum(dim=-1) / temperature
                    - maxlogit.squeeze(-1)
                )
                # print(additive)
                lognorm = (logits.exp().sum(dim=-1) + additive).log()
            ...
            #     loss = loss.sum(dim=-1)
                ...
                temperature = 1.0
                hidden_states = (
                    hidden_states[:, preflen - 1 : -1, :].contiguous().detach()
                )
                pred_embs = input_embeds[:, preflen:, :].contiguous()
                loss = -(hidden_states * pred_embs).sum(dim=-1) / temperature
                logits = hidden_states.matmul(embed_lut.weight.t()) / temperature
                ...
                C = (
                    (
                        torch.triu(torch.ones((pred_embs.size(1), pred_embs.size(1))))
                        - torch.eye(pred_embs.size(1), pred_embs.size(1))
                    )
                    .to(self.device)
                    .detach()
                )
    ...
    def compute_gold_loss(self, batch, **kwargs):
        ...
        if losstype == "xentropy":
            ...
            loss = F.nll_loss(
                lm_logprobs.squeeze(0), target.squeeze(0), reduction="none"
            ).sum(dim=-1)
            ...
            if losstype == "cosine":
                ...
                or losstype == "detachdot"
                or losstype == "detachdot2"
            ):
                hidden_states = hidden_states.contiguous()
                pred_embs = input_embeds[:, prompt.size(1) :, :].contiguous()
    ...
    def _postprocess_output(self, prepared_input, output_ids):
        return output_ids[
            :,
            prepared_input["input_ids"].size(1) :,
        ]
...
def top_k_top_p_filtering(
    ...
    if top_p > 0.0:
        ...
        indices_to_remove = sorted_indices_to_remove.scatter(
            dim=2, index=sorted_indices, src=sorted_indices_to_remove
        )
        # print(filter_value)
