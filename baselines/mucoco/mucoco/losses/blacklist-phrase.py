...
class KeywordL2Loss(BaseLoss):
    ...
    def compute_loss(self, batch, preds, **kwargs):
        ...
        # print(dist.size())
        dist = F.log_softmax(dist, dim=1).index_select(
            dim=1, index=self.keywords_ids[0]
        )
        # print(dist.size())
        # print(dist)
        ...
        try:
            ngram_nll = (
                F.conv2d(
                    dist.unsqueeze(1),
                    self.eye_filter[:, :, : self.ngram_length, : self.ngram_length],
                )
                / self.ngram_length
            )
    ...
    def process_keyword(self, keyword):
        ...
        if self.keyword == keyword:
            ...
            for (
                ...
                keyword_embeds = self.model.get_input_embeddings()(
                    self.keywords_ids[-1]
                ).detach()
                ...
                self.epsilon = (
                    ndist.mean(dim=0).topk(k=2, dim=0)[0][..., 1].item() - delta
                )
