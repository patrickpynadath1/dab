...
class KeyWordClassification(BaseLoss):
    ...
    def __init__(self, model, tokenizer, args):
        ...
        for k, v in self.topic2words.items():
            self.topic2words[k] = [
                torch.LongTensor(self.tokenizer.encode(w)).to(self.device) for w in v
            ]
        # self.topic2words = {}
        ...
        self.epsilon_additive = (
            0  # -np.log(self.topic2words[self.topic_target].sum().item())
        )
    ...
    def compute_loss(self, batch, preds, **kwargs):
        ...
        keyword_logits = -torch.square(
            torch.cdist(pred_embeds, embed_lut.weight.unsqueeze(0))
        )
        ...
        for keyword_tokenized in self.topic2words[topic]:
            keyword_probs = keyword_probs_all.index_select(
                dim=-1, index=keyword_tokenized
            )
            ngram_length = keyword_tokenized.size(0)
            ngram_probs = [
                keyword_probs[:, : output_length - ngram_length + 1, 0]
            ]  # first token of the ngram
            for i in range(1, ngram_length):
                ngram_probs.append(
                    keyword_probs[:, i : output_length - ngram_length + i + 1, i]
                )
