...
class UniqueNessLoss(BaseLoss):
    ...
    def compute_loss(self, batch, preds, **kwargs):
        ...
        input_logits = -torch.square(
            torch.cdist(input_embeds, embed_lut.weight.unsqueeze(0))
        )
        input_probs = torch.softmax(input_logits, dim=-1)
        # print(input_logits.size())

        C = (
            torch.tril(torch.ones((input_embeds.size(1), input_embeds.size(1))))
            - torch.diag(torch.ones(input_embeds.size(1)))
        ).to(self.device)
