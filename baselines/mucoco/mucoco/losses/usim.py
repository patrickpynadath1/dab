...
class USimLoss(BaseLoss):
    ...
    def compute_loss(self, batch, preds, **kwargs):
        ...
        gold_features = mean_pooling(
            self.model(input_ids=source),
            attention_mask=torch.ones(source.size(0), source.size(1)).to(self.device),
        )
        gold_features = (
            gold_features.detach()
        )  # don't need to pass gradients through this
        ...
        target_features = mean_pooling(
            self.model(inputs_embeds=target_embeds),
            attention_mask=torch.ones(target_embeds.size(0), target_embeds.size(1)).to(
                self.device
            ),
        )

        loss = 1.0 - (
            F.normalize(gold_features, dim=-1, p=2)
            * F.normalize(target_features, dim=-1, p=2)
        ).sum(dim=-1)
    ...
    def compute_gold_loss(self, batch, **kwargs):
        ...
        target_features = mean_pooling(
            self.model(input_ids=target),
            attention_mask=torch.ones(target.size(0), target.size(1)).to(self.device),
        )

        loss = 1.0 - (
            F.normalize(source_features, dim=-1, p=2)
            * F.normalize(target_features, dim=-1, p=2)
        ).sum(dim=-1)
