...
def get_parser():
    ...
    parser.add_argument(
        ...
        default=0,
        type=int,
        help="can be used to specify a subset of input file to decode (start index)",
    )
    parser.add_argument(
        "--end-idx",
        default=-1,
        type=int,
        help="can be used to specify a subset of input file to decode (last index)",
    )
    parser.add_argument(
        "--datastyle",
        default="text",
        type=str,
        choices=["jsonl", "single-jsonl", "text"],
        help="jsonl or plaintext (text)",
    )
    parser.add_argument(
        "--jsonl-primary-key",
        default="context",
        type=str,
        help="primary key to access text in the jsonl file",
    )
    parser.add_argument(
        "--jsonl-secondary-key",
        default=None,
        type=str,
        help="secondary key to access text in the jsonl file",
    )
    parser.add_argument(
        "--jsonl-tertiary-key",
        default=None,
        type=str,
        help="tertiary key to access text in the jsonl file",
    )
    parser.add_argument(
        "--jsonl-tokenized",
        default="false",
        type=str,
        choices=["true", "false"],
        help="the text in jsonl files is already tokenized if true",
    )
    parser.add_argument(
        "--additional-data",
        default=None,
        type=str,
        help="path of additional data used in some losses",
    )
    parser.add_argument(
        "--max-length",
        default=None,
        type=int,
        help="L: the sentence length you want to predict at every step. Use this for models which have no padding/trained while also predicting the padding. Not used in experiments reported in the paper",
    )
    parser.add_argument(
        "--max-output-length",
        default=None,
    ...
    parser.add_argument(
        ...
        default=1000,
        type=int,
        help="This is the max length that will fit into the GPU, max_length <= max_allowed_length",
    )
    parser.add_argument(
        "--length-diff",
        default="0",
        type=str,
        help="change the length of the target by adding these values",
    )
    parser.add_argument(
        "--restarts",
        default=0,
        type=int,
        help="If the constraints are not satisfied in a run, restart the optimization",
    )
    parser.add_argument(
        "--num_samples",
        default=1,
        type=int,
        help="number of times to run the decoding process for each input",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="use target specific tokenizer",
    )
    parser.add_argument(
        "--use_context",
        type=str,
        default="false",
        help="use context to compute loss (for some of them atleast)",
    )
    parser.add_argument(
        "--random-example",
        type=str,
    ...
    parser.add_argument(
        "--final-bias", action="store_true", help="use target specific tokenizer"
    )
    parser.add_argument(
        "--show-all-outputs", action="store_true", help="show all valid outputs"
    )
    parser.add_argument(
        "--debug-gradients", default="false", choices=["true", "false"], help=""
    )
    parser.add_argument(
        "--allow-diff-vocab", action="store_true", help="show all valid outputs"
    )
    parser.add_argument(
        "--linear-scale",
        default="false",
        choices=["true", "false"],
        help="use a linear scale instead of lagrange multipliers",
    )
    ...
    parser.add_argument(
        ...
        default="text",
        type=str,
        help="write output in jsonl or text format",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="none",
        help="use gold loss as epsilons",
    )
    parser.add_argument(
        "--custom-epsilons", type=str, default="none", help="use gold loss as epsilons"
    )
    ...
    parser.add_argument(
        ...
        default="false",
        choices=["true", "false"],
        help="sample when doing embedgd updates",
    )
    parser.add_argument(
        "--embedgd-top-k",
        default=0,
    ...
    parser.add_argument(
        "--expgd-do-sample", action="store_true", help="sample when doing expgd updates"
    )
    parser.add_argument(
        "--expgd-top-k",
        default=0,
    ...
    parser.add_argument("--repetition-penalty", default=0.0, type=float, help="")
    ...
    parser.add_argument(
        "--kweight", default=1.0, type=float, help="kweight. check gpt2.py"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--selection_criterion",
        default="primary_allsat",
        help="",
        choices=["weighted_sum", "primary_allsat", "last", "mrr_allsat"],
    )  # mrr = most recent repetition
    parser.add_argument(
        "--early-stop-steps",
        default=-1,
        type=int,
        help="stop if the output hasn't changed in this many steps and the constraints are satisfied",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="none",
        type=str,
        help="for classification losses, which is the label of interest",
    )
    ...
    parser.add_argument(
        "--half-lr",
        action="store_true",
        help="half the learning rate if the loss doesn't decrease for 20 steps",
    )
    parser.add_argument(
        "--always-mucoco",
        type=str,
        default="false",
        choices=["true", "false"],
        help="always use mucoco generated outputs (that is never pick beam search outputs even if the optimization fails)",
    )
    parser.add_argument(
        "--only-mucoco",
        type=str,
    ...
    parser.add_argument(
        "--same-embeds",
        action="store_true",
        help="use same embeddings for all models (used with target_type embeddings",
    )
    parser.add_argument(
        "--metric",
        default="dot",
        type=str,
        help="metric to compute NN for target_type embeddings",
    )
