...
def main(args):
    ...
    for i, model_path in enumerate(model_paths):
        if (
            ...
            if model_types[i] == "sentence-transformer":
                ...
                name2model[model_path] = getattr(
                    transformers, model_types[i]
                ).from_pretrained(
                    model_path, config=name2config[model_path], cache_dir=args.cache_dir
                )
            ...
            new_vocab_size = (
                name2model[model_path].get_input_embeddings().num_embeddings
            )
            ...
            if new_vocab_size != prev_vocab_size and prev_vocab_size is not None:
                if not args.allow_diff_vocab:
                    raise ValueError(
                        f"all models should have the same vocabulary {prev_vocab_size} != {vocab_size}"
                    )
                else:
                    logger.warning(
    ...
    assert (
        args.data is not None or args.additional_data is not None
    ), "no data path has been provided"
    ...
    for source_text, target_text, additional_text, shuffled_target_text in zip(
        source_dataset, target_dataset, additional_dataset, shuffled_target_dataset
    ):
        ...
        source_indices = primary_tokenizer.encode(source_text, return_tensors="pt").to(
            device
        )
        ...
        with torch.no_grad():
            predicted_indices = clean_output(
                lossfns[0]
                .generate(input_ids=input_ids, additional_ids=additional_indices)[0]
                .tolist(),
        ...
        if args.target_tokenize_different:
            with primary_tokenizer.as_target_tokenizer():
                beam_prediction = primary_tokenizer.decode(
                    predicted_indices[0].tolist()
                )
        else:
        ...
        if args.target_tokenize_different:
            with primary_tokenizer.as_target_tokenizer():
                ...
                shuffled_target_indices = primary_tokenizer.encode(
                    shuffled_target_text, return_tensors="pt", add_special_tokens=False
                ).to(device)
        else:
        ...
        if (
            ...
            for lossid in range(len(losses)):
                ...
                predicted_loss, predicted_lo = lossfns[lossid].compute_gold_loss(
            ...
            for lossid in range(len(losses)):
                lossname = losses[lossid]
                target_loss, target_lo = lossfns[lossid].compute_gold_loss(
            ...
            for lossid in range(len(losses)):
                lossname = losses[lossid]
                shuffled_target_loss, shuffled_target_lo = lossfns[
                    lossid
                ].compute_gold_loss(
                ...
                if "label_prediction" in shuffled_target_lo:
                    shuffled_target_labels[lossid] = shuffled_target_lo[
                        "label_prediction"
                    ]
                else:
            ...
            if args.outfile is not None:
                shuffled_targetf.write(
                    " ".join([str(x) for x in shuffled_targetlosses]) + "\n"
                )
