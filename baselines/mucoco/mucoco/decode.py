...
def main(args):
    ...
    if args.keywords is None or args.keywords == "none":
        ...
        if len(keywords) == 1:
            keywords = [
                f"_topic_:{args.keywords[0]}" for _ in losses
            ]  # when keyword isn't used but topic is passed
    ...
    for i, model_path in enumerate(model_paths):
        ...
        if (
            ...
            embed_luts.append(
                name2model[model_path].get_decoder().get_input_embeddings()
            )
        else:
    ...
    if source_dataset is None:
        ...
        if args.datastyle == "text":
            ...
            with open(
                context_data
            ) as csvfile:  # there can be multiple contexts, for example for paraphrasing, so we allow for a list of contexts for every input
        ...
        end_idx = (len(source_dataset) + args.end_idx) % len(
            source_dataset
        )  # also works with negative end_idx
            ...
            for sample_idx in range(args.num_samples):
                for restart_idx in range(
                        ...
                        with torch.no_grad():
                            ...
                            AR_predicted_indices = clean_output(
                                ...
                                .generate(
                                    ...
                                    additional_ids=additional_batch[batchidx].unsqueeze(
                                        0
                                    ),
                                )[0]
                                .tolist(),
                    ...
                    if not definite_skip:
                        ...
                        if (
                            args.max_length is None or args.max_length == -1
                        ...
                        for sent_length_ in length_range:
                                    ...
                                    with torch.cuda.amp.autocast():
                                        ...
                                        if (
                                            ...
                                            for sid in range(
                                                ...
                                                closs_for_theta = lambda_.get_loss(
                                                    sid - 1,
                                                    damp * mask,
                                                    (
                                                        cur_epsilon
                                    ...
                                    for b in range(batch_size):
                                        ...
                                        if modify_condition:
                                            if args.dynamic_lr_update:
                                                print(
                                                    "resetting the learning rate, a constraint has been satisfied"
                                                )
                                                cur_lr = optimizer._optimizer.update_lr(
                                                    args.lr
                                                )
                            ...
                            for b, item in enumerate(best_pred_tokens):
                                ...
                                if (
                                    ...
                                    print(
                                        f"best prediction is from beam search, all constraints were not satisfied, allsat={lengthwise_best_prediction[b][2]}"
                                    )
                                else:
                                    prediction_ids = ", ".join(
                        ...
                        if ask_skip != "y":
                            if args.debug:
                                print(
                                    "Skipping this example. the beam search output already satisfies all the constraints or there's no constraints"
                                )
                                for b in range(batch_size):
                                    print(
                                        "best prediction for all lengths: ",
