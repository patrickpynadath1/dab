...
class TargetSimplex(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # )
        self._pred_logits = nn.Parameter(
            torch.Tensor(batch_size, sent_length, vocabsize).to(device)
        )
    ...
    def forward(
        self, content_embed_lut, style_embed_lut=None, tokenizer=None, debug=False
    ):
        ...
        if debug:
            ...
            print(torch.exp(self._pred_logits).eq(1.0).all())
            print(index)
            input()

        source_pred_emb = (pred_probs.unsqueeze(-1) * content_embed_lut.weight).sum(
            dim=-2
        )
        target_pred_emb = None
        if style_embed_lut is not None:
            target_pred_emb = (pred_probs.unsqueeze(-1) * style_embed_lut.weight).sum(
                dim=-2
            )
        return (
            (source_pred_emb, target_pred_emb, softmax_pred_probs),
            predictions,
            pred_probs,
        )
    ...
    def forward_multiple(self, embed_luts, **kwargs):
        ...
        if self.sampling_strategy == "greedy":
            ...
            index = torch.multinomial(
                probabilities.view(-1, filtered_logits.size(-1)), 1
            )
            ...
            raise ValueError(
                "wrong decode method. If you want to do beam search, change the objective function"
            )
        ...
        return (
            (pred_embs,),
            predictions,
            (pred_probs, softmax_pred_probs),
        )  # pred_probs is actually just logits
    ...
    def initialize(self, random_init=False, init_value=None):
        if init_value is not None:
            ...
            print(1 - eps - eps / (V - 1))
            print(eps / (V - 1))
            init_value = torch.zeros_like(self._pred_logits).scatter_(
                -1, init_value.unsqueeze(2), 1.0 - eps - eps / (V - 1)
            )
...
class TargetProbability(nn.Module):  # this is the class used in the final results
    ...
    def forward_multiple(self, embed_luts, **kwargs):
        ...
        #     predictions = index.squeeze(-1)
            ...
            next_token = (
                torch.empty((pred_logits.size(0), pred_logits.size(1)))
                .long()
                .to(pred_logits.device)
            )
            for i in range(pred_logits.size(1)):
                filtered_logits = top_k_top_p_filtering(
                    pred_logits[:, i, :] / temperature, self.top_k, self.top_p
                )
            ...
            if embed_lut.weight.size(0) > pred_probs.size(2):
                pred_embs.append(
                    (
                        ...
                        * embed_lut.weight[: pred_probs.size(2), :]
                    ).sum(dim=-2)
                )
            elif embed_lut.weight.size(0) < pred_probs.size(2):
                pred_embs.append(
                    (
                        pred_probs[:, :, : embed_lut.weight.size(0)].unsqueeze(-1)
                        * embed_lut.weight
                    ).sum(dim=-2)
                )
            else:
                pred_embs.append(
                    (pred_probs.unsqueeze(-1) * embed_lut.weight).sum(dim=-2)
                )
    ...
    def initialize(self, random_init=False, init_value=None):
        if init_value is not None:
            ...
            random_init
        ):  # sample a simplex from a dirichlet distribution for each token probability
            p = torch.distributions.dirichlet.Dirichlet(
                10000 * torch.ones(self._pred_probs.size(-1))
            )
...
class TargetEmbeddings(nn.Module):
    def __init__(
        ...
        self._pred_embeds = nn.Parameter(
            torch.Tensor(batch_size, sent_length, embed_dim).to(device)
        )
        self.device = device
        self.st = st  # straight-through or not
        self.sampling_strategy = sampling_strategy
        ...
        if self.metric == "cosine":
            self.tgt_emb = torch.nn.functional.normalize(
                embed_lut.weight.data, p=2, dim=-1
            )
        else:
    ...
    def forward_multiple(self, embed_luts, new_predictions=None, **kwargs):
        ...
        if predictions is None:
            pred_logits = _emb_to_scores(
                self.metric, self._pred_embeds, self.tgt_emb, self.final_bias
            )
            pred_probs = (
                pred_logits  # F.softmax(pred_logits / self.temperature, dim=-1)
            )
        ...
        if self.st:
            ...
            for embed_lut, embed_scale in zip(embed_luts, self.embed_scales):
                ...
                # print(replace_mask)
                self._pred_embeds.data.copy_(
                    (
                        replace_mask * n_pred_embs
                        + (1.0 - replace_mask) * self._pred_embeds
                    ).data
                )
                pred_embs.append(
                    self._pred_embeds + (n_pred_embs - self._pred_embeds).detach()
                )
                # input()
        else:
            for embed_lut, embed_scale in zip(embed_luts, self.embed_scales):
    ...
    def initialize(
        ...
        _, self._pred_ids = _emb_to_scores(
            self.metric, self._pred_embeds, self.tgt_emb, self.final_bias
