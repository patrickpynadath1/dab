...
def main(args):
    ...
    if "allsat" in args.selection_criterion:
        ...
        args.selection_criterion == "weighted_sum" and args.betas is not None
    ) or args.selection_criterion == "last":
    ...
    assert (
        len(betas) == len(losses)
        and len(losses) == len(model_paths)
        and len(model_paths) == len(model_types)
        and len(betas) == len(lossabbr)
    )
    ...
    for i, model_path in enumerate(model_paths):
        if (
            ...
            name2tokenizer[model_path] = AutoTokenizer.from_pretrained(
                tokenizer_paths[i], cache_dir=args.cache_dir, use_fast=True
            )
            name2config[model_path] = AutoConfig.from_pretrained(
                model_path_, cache_dir=args.cache_dir
            )

            if model_types[i] == "sentence-transformer":
                name2model[model_path] = lossbuilder.ModelWrapper(
                    SentenceTransformer(model_path_)
                )
            elif "Custom" in model_types[i]:
                name2model[model_path] = lossbuilder.ModelWrapper(
                    getattr(utils, model_types[i]).from_pretrained(
                        model_path_,
                ...
                if second_model_path_ is None:
                    model_ = lossbuilder.ModelWrapper(
                        getattr(transformers, model_types[i]).from_pretrained(
                            model_path_,
                            config=name2config[model_path],
                            cache_dir=args.cache_dir,
                        )
                    ...
                    model2_ = lossbuilder.ModelWrapper(
                        getattr(transformers, model_types[i]).from_pretrained(
                            second_model_path_,
                            config=name2config[model_path],
                            cache_dir=args.cache_dir,
            ...
            if not args.show_warnings:
                ...
                if isinstance(name2model[model_path], tuple):
                    set_global_logging_level(
                        logging.ERROR, [name2model[model_path][0].__module__]
                    )
                    set_global_logging_level(
                        logging.ERROR, [name2model[model_path][1].__module__]
                    )
                else:
                    set_global_logging_level(
                        logging.ERROR, [name2model[model_path].__module__]
                    )
            ...
            if new_vocab_size != prev_vocab_size and prev_vocab_size is not None:
                if not args.allow_diff_vocab:
                    raise ValueError(
                        f"all models should have the same vocabulary {new_vocab_size} != {vocab_size}"
                    )
                else:
                    logger.warning(
                        "all models don't have the same vocabulary and we are still proceeding"
                    )
            prev_vocab_size = vocab_size

        if (
            args.target_tokenize_different
        ):  # for seq2seq models where target tokenizer is different than the source tokenizer
        ...
        if (
            getattr(model_, "get_decoder", None) is None
    ...
    for i, loss in enumerate(losses):
        ...
        lossfns.append(
            lossbuilder.build_loss(
                loss, name2model[model_paths[i]], name2tokenizer[model_paths[i]], args
            )
        )
        print(lossfns[-1])
    ...
    if (
        args.model_dtype == "fp16"
    ):  # while this is supported it doesn't work that well yet. Not recommended
        for name, model in name2model.items():
            model.half()
    ...
    if args.epsilons is not None and args.epsilons != "none":
        ...
        if args.min_epsilons is not None:
            ...
            epsilon_warmup_steps = [
                int(steps) for steps in args.epsilon_warmup_steps.split(":")
            ]
            epsilon_cooldown_steps = [
                int(steps) for steps in args.epsilon_cooldown_steps.split(":")
            ]
            epsilon_decay_functions = [
                f for f in args.epsilon_decay_functions.split(":")
            ]
        else:
        ...
        min_epsilons = [
            eps + getattr(lossfns[i + 1], "epsilon_additive", 0)
            for i, eps in enumerate(min_epsilons)
        ]
    else:
        epsilons = []
        min_epsilons = []
    ...
    if source_dataset is None:
        ...
        if args.datastyle == "text":
            ...
            source_dataset = [
                json.loads(l)[args.jsonl_primary_key] for l in open(source_data)
            ]
            target_dataset = [
                json.loads(l)[args.jsonl_primary_key] for l in open(target_data)
            ]
            additional_dataset = [
                json.loads(l)[args.jsonl_primary_key] for l in open(additional_data)
            ]
            if (
                args.jsonl_secondary_key is not None
                and args.jsonl_secondary_key != "none"
            ):
                ...
                additional_dataset = [
                    x[args.jsonl_secondary_key] for x in additional_dataset
                ]

            context_dataset = [None] * len(source_dataset)
            if args.use_context:
                context_dataset = [
                    json.loads(l)[args.jsonl_primary_key] for l in open(context_data)
                ]
                if (
                    args.jsonl_secondary_key is not None
                    and args.jsonl_secondary_key != "none"
                    ...
                    context_dataset = [
                        x[args.jsonl_secondary_key] for x in context_dataset
                    ]
            ...
            target_dataset = [
                json.loads(l)[args.jsonl_secondary_key] for l in open(target_data)
            ]
            # additional_dataset = [json.loads(l)[args.jsonl_tertiary_key] for l in open(additional_data)]
            additional_dataset = [
                json.loads(l)[args.jsonl_secondary_key] for l in open(additional_data)
            ]

            context_dataset = [None] * len(source_dataset)
            if args.use_context:
                context_dataset = [
                    [json.loads(l)[args.jsonl_secondary_key]]
                    for l in open(context_data)
                ]  # maybe meaningful
        start_idx = args.start_idx
        end_idx = (len(source_dataset) + args.end_idx) % len(
            source_dataset
        ) + 1  # also works with negative end_idx
    ...
    batch_size = (
        args.batch_size
    )  # higher than 1 batch size does not work at the moment. It won't fit in a single GPU anyway
    ...
    for text_id, source_text in enumerate(source_dataset):
        ...
        if (
            args.num_examples > 0 and c > 0 and c == args.num_examples
        ):  # stop after processing num_examples if it is set
            print(f"done {c}")
            break
        ...
        # edits for BOLT keyword gen here
            ...
            keywords = (
                ["none"] + additional_text.split(", ") + ["none"] + ["\n", "\n\n"]
            )
            # input(keywords)
            ...
            if args.datastyle == "text":
                keywords = (
                    ["none"]
                    + json.loads(additional_text)["concept_set"].split("#")
                    + ["none"]
                )
            else:
                print(additional_text)
            ...
            keywords = (
                ["none"]
                + ["#".join(words) for words in eval(additional_text)]
                + ["none"]
            )
            ...
            keywords = [
                word
                for i, word in enumerate(additional_text.strip().split("\t")[2:])
                if i % 2 == 1
            ]  # words at index 1, 3, 5 are translations in target
        ...
        if args.debug:
            early_skip = input(
                f"skip this example? {source_text} [yes(y)/maybe(m)/no(n)]"
            )
            if early_skip == "y":
                continue
        ...
        if not args.jsonl_tokenized:
            if source_text == "":
                ...
                source_indices = torch.LongTensor(
                    [[primary_tokenizer.bos_token_id]]
                ).to(device)
            elif "commongen" in args.keywords:
                source_indices = primary_tokenizer.encode(
                    " " + source_text,
                    return_tensors="pt",
                    truncation=True,
                    max_length=getattr(primary_config, "max_position_embeddings", None),
                ).to(device)
            ...
            if isinstance(additional_text, list):
                additional_indices = primary_tokenizer.encode(
                    additional_text[0], return_tensors="pt", add_special_tokens=False
                ).to(device)
            else:
                additional_indices = primary_tokenizer.encode(
                    additional_text, return_tensors="pt", add_special_tokens=False
                ).to(device)
            ...
            if args.target_tokenize_different:
                ...
                context_indices = (
                    primary_tokenizer.encode(
                        context_texts[0], return_tensors="pt", add_special_tokens=False
                    )
                    .to(device)
                    .unsqueeze(1)
                )
            ...
            if not args.target_tokenize_different and "Seq2SeqLM" in model_paths[0]:
                logger.warning(
                    "you are using a seq2seq model for your primary loss but not tokenizing the target sentences with a different target tokenizer."
                )
            ...
            if args.target_tokenize_different:
                with primary_tokenizer.as_target_tokenizer():
                    for_predicted_source_indices = primary_tokenizer.encode(
                        source_text, return_tensors="pt"
                    ).to(device)
                    target_indices = primary_tokenizer.encode(
        ...
        print(source_text)
        if (
            len(source_batch) == batch_size
        ):  # this is just one for now, greater than 1 batch size will not work
            ...
            for_predicted_source_batch = torch.cat(
                for_predicted_source_batch, dim=0
            ).to(device)
            ...
            for batchidx in range(source_batch.size(0)):  # batch size is 1
                with torch.no_grad():
                    starttime = time.time()
                    AR_predicted_all = lossfns[0].generate(
                    ...
                    for sample_idx in range(len(AR_predicted_all)):
                        AR_predicted_indices = clean_output(
                            AR_predicted_all[sample_idx].tolist(),
                            eos_token_id=eos_token_id,
                            return_tensors=True,
                            allow_first_eos=losses[0] == "bart",
                            remove_first=losses[0] == "marianmt",
                        ...
                        if args.target_tokenize_different:
                            with primary_tokenizer.as_target_tokenizer():
                                AR_prediction = primary_tokenizer.decode(
                                    AR_predicted_indices[0].tolist()
                                )
                        else:
                            AR_prediction = primary_tokenizer.decode(
                                AR_predicted_indices[0].tolist()
            ...
            for sample_idx in range(args.num_samples):
                ...
                for restart_idx in range(
                    args.restarts + 1
                ):  # restart the optimization if the constraints are not satisfied
                    predicted_batch = predicted_batches[
                        (sample_idx * (args.restarts + 1) + restart_idx)
                        % len(predicted_batches)
                    ]
                    AR_prediction = AR_prediction_all[
                        (sample_idx * (args.restarts + 1) + restart_idx)
                        % len(predicted_batches)
                    ]
                    ...
                    if args.debug:
                        print(
                            "AR output:", source_text, additional_text, predicted_batch
                        )
                    ...
                    for lossid in range(len(losses)):
                        predicted_loss, predicted_lo = lossfns[
                            lossid
                        ].compute_gold_loss(
                        ...
                        if (
                            lossid > 0 and args.custom_epsilons[lossid - 1] == "true"
                        ):  # use a custom epsilon value defined by the loss class (used with ngram-inverse)
                            min_epsilons[lossid - 1] = getattr(
                                lossfns[lossid], "epsilon", 0
                            )
                            epsilons[lossid - 1] = (
                                100  # (lossfns[lossid], "epsilon", 0)
                            )
                        ...
                        if lossid > 0:
                            predicted_allsat = predicted_allsat and (
                                predicted_loss <= min_epsilons[lossid - 1]
                            )
                        ...
                        if (
                            lossid > 0 and args.gold_loss_epsilons[lossid - 1] == "true"
                        ):  # use the predicted loss as the threshold, mucoco has to beat it then
                            min_epsilons[lossid - 1] = predicted_loss + getattr(
                                lossfns[lossid], "epsilon_additive", 0
                            )
                            epsilons[lossid - 1] = predicted_loss + getattr(
                                lossfns[lossid], "epsilon_additive", 0
                            )  ##TODO check
                    ...
                    if args.only_mucoco == "false":
                        lengthwise_best_prediction = [
                            (
                                AR_prediction,
                                total_predicted_loss,
                                predicted_allsat,
                                predicted_batch[0].tolist(),
                                -1,
                            )
                        ]
                    ...
                    if (
                        args.debug
                        and lengthwise_best_prediction[0] is not None
                        and lengthwise_best_prediction[0][2]
                    ):
                    ...
                    if args.debug and early_skip == "m":
                        print(
                            f"new example: {source_text}\nautoregressive output: {AR_prediction}"
                        )
                        for lossid in range(len(losses)):
                            print(
                                f"{lossabbr[lossid]} for desired label_id({label_ids[lossid]}): {predictedlosslists[-1][lossid]}; predicted label: {predicted_labels[lossid]}"
                            )
                        if predicted_allsat:
                            print(
                                f"autoregressive output already satisfies the constraints"
                            )
                    ...
                    if not definite_skip:
                        ...
                        if (
                            args.length_diff is not None and args.length_diff != "none"
                        ) and args.init not in ["source", "target"]:
                            ...
                            length_range = [
                                predicted_length + int(diff)
                                for diff in args.length_diff.split(":")
                            ]
                            length_range = [
                                x
                                for x in length_range
                                if x <= args.max_allowed_length and x >= 1
                            ]
                        ...
                        for sent_length_ in length_range:
                            ...
                            if args.prefix_length > 0:
                                ...
                                target_prefix = (
                                    torch.empty((source_indices.size(0), 0))
                                    .long()
                                    .to(device)
                                )
                            ...
                            if sent_length > args.max_allowed_length:
                                ...
                                print(
                                    f"changed output length to {sent_length} from {old_l} to avoid GPU overflow. This is a temporary solution"
                                )
                            else:
                                print("predicting a sentence length: ", sent_length)

                            if (
                                args.target_type == "simplex"
                            ):  # use V sized real vector for each token and apply softmax before output
                                init_value = None
                                break_after = False
                                ...
                                args.target_type == "probs"
                            ):  # use V sized vector which sums to one for each token and apply softmax before output
                                init_value = None
                                break_after = False
                                if (
                                    args.init == "source"
                                ):  # initialize the target with the source
                                    ...
                                    args.init == "target"
                                ):  # initialize the target with the autoregressive output
                                ...
                                if (
                                    ...
                                    args.init == "targettarget"
                                ):  # initialize the target with given target
                                    ...
                                    random_indices = (
                                        torch.multinomial(
                                            ...
                                            / primary_vocab_size,
                                            num_samples=batch_size * sent_length,
                                            replacement=True,
                                        )
                                        .view(batch_size, sent_length)
                                        .to(device)
                                    )
                                    ...
                                    if args.target_tokenize_different:
                                        with primary_tokenizer.as_target_tokenizer():
                                            indices = (
                                                torch.empty((batch_size, sent_length))
                                                .long()
                                                .fill_(primary_tokenizer.eos_token_id)
                                                .to(device)
                                            )
                                    else:
                                    ...
                                    indices = (
                                        torch.zeros((batch_size, sent_length))
                                        .long()
                                        .to(device)
                                    )
                                    init_value = embed_luts[0](indices)
                            ...
                            best_losses = [
                                [None] * batch_size for _ in range(len(losses))
                            ]
                            best_step = -100
                            ...
                            for step in range(args.optim_steps):
                                try:
                                    with torch.cuda.amp.autocast(enabled=False):
                                        ...
                                        pred_embeds, pred_tokens, pred_probs = (
                                            outputs.forward_multiple(
                                                embed_luts,
                                                new_predictions=getattr(
                                        ...
                                        if not args.time and args.debug:
                                            ...
                                            target_sents = get_sent(
                                                torch.cat(
                                                    [target_prefix, pred_tokens], dim=1
                                                ),
                                                primary_tokenizer,
                                            )
                                        ...
                                        # print("what", args.use_context)
                                        for lossid, lossname in enumerate(losses):
                                            lossvalue, logging_output = lossfns[
                                                lossid
                                            ].compute_loss(
                                                [source_batch, target_prefix],
                                                [
                                                    pred_tokens,
                                                    pred_embeds[0][lossid],
                                                    pred_probs,
                                                ],
                                                additional_batch=additional_batch,
                                                context_batch=context_batch,
                                            ...
                                            losslists[lossid][-1].append(
                                                lossvalue.sum().item()
                                            )  # for logging
                                            losses_for_backward.append(
                                                lossvalue
                                            )  # for backward
                                            logging_outputs.append(logging_output)

                                        if (
                                            args.linear_scale == "true"
                                        ):  # no lagragian, plain old linear sum
                                            #
                                            # grads = []
                                            ...
                                            cur_epsilons = (
                                                []
                                            )  # just for avoiding syntax errors, epsilons are useless in this setting
                                            for sid in range(len(losses_for_backward)):
                                                total_loss = (
                                                    total_loss
                                                    + betas[sid]
                                                    * losses_for_backward[sid]
                                                )
                                            ...
                                            constraint_values = []
                                            for sid in range(
                                                1, len(losses_for_backward)
                                            ):  # the secondary losses or constraints

                                                cur_epsilon = get_epsilon(
                                                    step,
                                                    epsilons[sid - 1],
                                                    min_epsilons[sid - 1],
                                                    epsilon_warmup_steps[sid - 1],
                                                    epsilon_cooldown_steps[sid - 1],
                                                    epsilon_decay_functions[sid - 1],
                                                )
                                        ...
                                        if (
                                            ...
                                            for i in range(len(g)):
                                                ...
                                                for j in range(len(g)):
                                                    ...
                                                    s = g[i] + g[j]
                                                    cosine = dot / (
                                                        torch.norm(g[i], dim=-1, p=2)
                                                        * torch.norm(g[j], dim=-1, p=2)
                                                    )
                                                    snorm = s.data.norm(2, -1)
                                                    print(i, j, dot, cosine, s)
                                            print(
                                                "total sum norm",
                                                totals.data.norm(2, -1),
                                            )
                                    ...
                                    if args.linear_scale != "true" and len(losses) > 1:
                                        sats = (
                                            torch.Tensor(constraint_values)
                                            .ge(0.0)
                                            .to(device)
                                        )
                                        update_lambda_condition = (
                                            step % args.lambda_update == 0
                                        )
                                        lambda_mask = float(
                                            update_lambda_condition
                                        ) * torch.ones_like(sats)
                                    ...
                                    if (
                                        dynamic_lambda_update_prev_loss is not None
                                        and abs(
                                            total_batchlossitem
                                            - dynamic_lambda_update_prev_loss
                                        )
                                        <= 1e-6
                                    ):
                                        repeat_counts[0] += 1
                                        if (
                                            args.linear_scale != "true"
                                            and len(losses) > 1
                                            and args.dynamic_lambda_update
                                        ):
                                            lambda_mask = 1 - sats.float()
                                        ...
                                        if (
                                            args.dynamic_lr_update
                                            and best_allsat[0] is not None
                                            and best_allsat[0]
                                        ):
                                            update_lr_condition = "increase"
                                            cur_lr = optimizer._optimizer.update_lr(
                                                min(
                                                    cur_lr + args.lr_update_size,
                                                    args.max_lr,
                                                )
                                            )
                                    else:
                                    ...
                                    if args.linear_scale != "true" and len(losses) > 1:
                                        ...
                                        optimizer_lambda._optimizer.set_mask(
                                            lambda_mask.clamp(max=1.0, min=0.0)
                                        )
                                    ...
                                    if args.keywords in topics:
                                        ...
                                        if has_kw:
                                            with open(
                                                f"mucola_kw_{args.keywords}.txt", "a"
                                            ) as f:
                                    ...
                                    for b in range(batch_size):
                                        cur_loss = 0.0
                                        for beta, lossval in zip(
                                            betas, losses_for_backward
                                        ):
                                            cur_loss = (
                                                cur_loss + beta * lossval[b].item()
                                            )
                                        ...
                                        modify_condition = (
                                            ...
                                            or (
                                                best_loss[b] is None
                                                and args.selection_criterion
                                                == "weighted_sum"
                                            )
                                            or (
                                                best_loss[b] is not None
                                                and args.selection_criterion
                                                == "weighted_sum"
                                                and best_loss[b] > cur_loss
                                            )
                                        )

                                        # print(repeat_counts, allsat, best_loss, best_allsat)
                                        if (
                                            not modify_condition
                                            and args.selection_criterion == "mrr_allsat"
                                        ):
                                            modify_condition = (
                                                best_loss[b] is None
                                                and allsat
                                                and repeat_counts[b] == 5
                                            ) or (
                                                best_loss[b] is not None
                                                and best_allsat[b]
                                                and allsat
                                            ...
                                            modify_condition = (
                                                (best_loss[b] is None and allsat)
                                                or (
                                                    best_loss[b] is not None
                                                    and not best_allsat[b]
                                                    and allsat
                                                )
                                                or (
                                                    best_allsat[b]
                                        ...
                                        if modify_condition:
                                            if args.dynamic_lr_update:
                                                print(
                                                    "resetting the learning rate and noise std, a constraint has been satisfied"
                                                )
                                                cur_lr = optimizer._optimizer.update_lr(
                                                    args.lr
                                                )
                                            ...
                                            for i in range(len(losses)):
                                                best_losses[i][b] = losses_for_backward[
                                                    i
                                                ][b].item()
                                            ...
                                            if (
                                                args.show_all_outputs
                                                and len(losses) > 1
                                                and allsat
                                            ):
                                                best_prediction_set[b].add(
                                                    target_sents[b]
                                                )
                                    ...
                                    if (
                                        ...
                                        if len(losses) > 1:
                                            ...
                                            for i in range(len(losslists)):
                                                log = (
                                                    log
                                                    + f" {lossabbr[i]}:{losslists[i][-1][-1]:.4f}; "
                                                )

                                            if best_loss[0] is not None:
                                                log = (
                                                    log[:-1]
                                                    + f"] |||| best [cur_loss:{sum(best_loss):.4f}; cons:{best_constrained};  "
                                                )
                                                for i in range(len(best_losses)):
                                                    log = (
                                                        log
                                                        + f"{lossabbr[i]}:{sum(best_losses[i]):.4f}; "
                                                    )
                                                log = (
                                                    log[:-1]
                                                    + f"@ step #{best_index[-1]}"
                                                ...
                                                log = (
                                                    log[:-1]
                                                    + f"] |||| best [none of the generations so far satisfies constraints]"
                                                )
                                            print(log)
                                        else:
                                    ...
                                    if (
                                        ...
                                        early_stop_condition = (
                                            (
                                                "allsat" in args.selection_criterion
                                                and best_allsat[0]
                                            )
                                            or (
                                        ...
                                        if (
                                            ...
                                            and abs(cur_loss - prev_loss) <= 1e-6
                                        ):
                                            same_loss_count += 1
                                        else:
                                            same_loss_count = 0
                                        ...
                                        if (
                                            early_stop_condition
                                            and same_loss_count >= args.early_stop_steps
                                        ):
                                            print(
                                                f"Early stop at @{step} with a loss value of {cur_loss} and satisfied constraints"
                                            )
                                            break
                                        elif (
                                            same_loss_count
                                            >= args.early_stop_steps + 100
                                        ):  # 2 * args.lambda_update:
                                            print(
                                                f"Early stop at @{step} with a loss value of {cur_loss} and unsatisfied constraints"
                                            )
                                            break
                                    ...
                                    print(
                                        "skipping remaining optimizing steps and showing the best option so far"
                                    )
                                    broken = True
                                    break
                            ...
                            for b, item in enumerate(best_pred_tokens):
                                if item is None and broken:
                                    ...
                                    if broken:
                                        broken_skip = input(
                                            "Skip this input entirely? yes(y)/no(continue)/press ctrl+c to exit"
                                        )
                                        broken_skip = broken_skip.lower() == "y"
                                        break
                                if (
                                    args.only_mucoco == "false" and not best_allsat[b]
                                ) or (
                                    item is None
                                ):  # item is none happens when optimization fails
                                    prediction_ids = ", ".join(
                                        [
                                            str(idx)
                                            for idx in AR_predicted_indices[0].tolist()
                                        ]
                                    )
                                    prediction_indices = AR_predicted_indices[
                                        0
                                    ].tolist()
                                    ...
                                    for lossid in range(len(betas)):
                                        lossvalue += (
                                            betas[lossid]
                                            * predictedlosslists[-1][lossid][b]
                                        )  # VERIFICATION NEEDED
                                    if lengthwise_best_prediction[b] is not None:
                                        print(
                                            f"{text_id} best prediction is from beam search, all constraints were not satisfied, allsat={lengthwise_best_prediction[b][2]}"
                                        )
                                    else:
                                        print(
                                            f"{text_id} best prediction is empty since you set the option (only_mucoco=True) to never predict autoregressive outputs, all constraints were not satisfied, allsat=False"
                                        )
                                else:
                                    prediction_ids = ", ".join(
                                        [str(x) for x in target_prefix[b].tolist()]
                                    )
                                    prediction_ids += f'[{", ".join([str(x) for x in item.tolist()])}]'
                                    prediction_indices = (
                                        target_prefix[b].tolist() + item.tolist()
                                    )

                                    targets = clean_output(
                                        item.tolist(),
                                        primary_tokenizer.eos_token_id,
                                        allow_first_eos=losses[0] == "bart",
                                    )  # , prompt=source_batch[b].unsqueeze(0), sentence_complete=True, lossfn=lossfns[0])
                                    if args.target_tokenize_different:
                                        with primary_tokenizer.as_target_tokenizer():
                                            prediction = primary_tokenizer.decode(
                                                target_prefix[b].tolist() + targets
                                            )
                                    else:
                                        prediction = primary_tokenizer.decode(
                                            target_prefix[b].tolist() + targets
                                    ...
                                    print(
                                        f"{text_id} best prediction at step",
                                        best_index[b],
                                    )
                                    lossvalue = best_loss[b]

                                    modify_condition = lengthwise_best_prediction[
                                        b
                                    ] is None or (
                                        args.selection_criterion == "weighted_sum"
                                        and lengthwise_best_prediction[b][1] > lossvalue
                                    )

                                    if (
                                        not modify_condition
                                        and args.selection_criterion == "primary_allsat"
                                    ):
                                        modify_condition = (
                                            not lengthwise_best_prediction[b][2]
                                            and best_allsat[b]
                                        ) or (
                                            lengthwise_best_prediction[b][2]
                                            and best_allsat[b]
                                            and lengthwise_best_prediction[b][1]
                                            > lossvalue
                                        )
                                        ...
                                        modify_condition = (
                                            not lengthwise_best_prediction[b][2]
                                            and best_allsat[b]
                                            and best_repeat_count[b] >= 2
                                        ) or (
                                            lengthwise_best_prediction[b][2]
                                            and lengthwise_best_prediction[b][4] >= 2
                                            and lengthwise_best_prediction[b][1]
                                            > lossvalue
                                        )
                                    ...
                                    if modify_condition:
                                        if args.debug:
                                            print(
                                                "modify condition satisfied", end="\n"
                                            )
                                        else:
                                            outallsatf.write(
                                                "modify_condition satisfied "
                                            )
                                        lengthwise_best_prediction[b] = (
                                            prediction,
                                            lossvalue,
                                            best_allsat[b],
                                            prediction_indices,
                            ...
                            if args.debug and not skip_printing:
                                for i, item in enumerate(best_pred_tokens):
                                    ...
                                    print(
                                        f"Prediction: {prediction}; repeated {best_repeat_count[b]} times"
                                    )
                                    print(
                                        "All generations that satisfied the constraints: ",
                                        best_prediction_set[i],
                                    )
                                    ...
                                    for lossid in range(len(losses)):
                                        out.append(
                                            f"{losses[lossid]}: {best_losses[lossid][i]}"
                                        )
                                    print("; ".join(out))
                        ...
                        b = 0
                        if (
                            lengthwise_best_prediction[b] is None
                            or not lengthwise_best_prediction[b][2]
                        ):  # constraints are not satisfied
                            if (
                                restart_idx < args.restarts
                            ):  # atleast one more restart is left
                                continue  # skip printing and loop over
                            elif lengthwise_best_prediction[b] is None:
                                lengthwise_best_prediction = [
                                    ("", -1, False, [], -1)
                                ]  # just blank which didn't satisfy the constraints
                        ...
                        if args.debug:
                            if not skip_printing:
                                for b in range(batch_size):
                                    print(
                                        ...
                                        f"repeat count: {lengthwise_best_prediction[b][4]}",
                                        "best prediction for all lengths: ",
                                        lengthwise_best_prediction[b][0]
                                        .strip()
                                        .replace("\n", " ")
                                        + "\n",
                                    )
                            ...
                            if args.output_style == "text":
                                for b in range(batch_size):
                                    ...
                                    outf.flush()
                                    outallsatf.write(
                                        str(lengthwise_best_prediction[b][2]) + "\n"
                                    )
                                    outallsatf.flush()
                                ...
                                for b in range(batch_size):
                                    if sample_idx == 0:
                                        output = {
                                            ...
                                            "generations": [
                                                {
                                                    "text": lengthwise_best_prediction[
                                                        b
                                                    ][0],
                                                    "tokens": lengthwise_best_prediction[
                                                        b
                                                    ][
                                                        3
                                                    ],
                                                    "allsat": lengthwise_best_prediction[
                                                        b
                                                    ][
                                                        2
                                                    ],
                                                    "repeat_count": lengthwise_best_prediction[
                                                        b
                                                    ][
                                                        4
                                                    ],
                                        ...
                                        if (
                                            "commongen" in args.keywords
                                            or "roc" in args.keywords
                                            or "iate" in keywords
                                        ):
                                        ...
                                        output["generations"].append(
                                            {
                                                "text": lengthwise_best_prediction[b][
                                                    0
                                                ],
                                                "tokens": lengthwise_best_prediction[b][
                                                    3
                                                ],
                                                "allsat": lengthwise_best_prediction[b][
                                                    2
                                                ],
                                                "repeat_count": lengthwise_best_prediction[
                                                    b
                                ...
                                for b in range(batch_size):  # for rouge
                                    if sample_idx == 0:
                                        output = {
                                            ...
                                            "summary": {
                                                ...
                                                "tokens": [
                                                    lengthwise_best_prediction[b][3]
                                                ],
                                                "allsat": [
                                                    lengthwise_best_prediction[b][2]
                                                ],
                                            ...
                                            "reference": {"text": target_text},
                                        }
                                        # print(output)

                                    else:
                                        output["summary"]["text"].append(
                                            lengthwise_best_prediction[b][0]
                                        )
                                        output["summary"]["tokens"].append(
                                            lengthwise_best_prediction[b][3]
                                        )
                                        output["summary"]["allsat"].append(
                                            lengthwise_best_prediction[b][2]
                                        )
                                    ...
                                    if args.keywords == "_cnndm_":
                                        if "keywords" in output["summary"]:
                                            output["summary"]["keywords"].append(
                                                additional_text
                                            )
                                        else:
                                ...
                                print(
                                    f"SAMPLE IDX: {sample_idx}, NUM SAMPLES: {args.num_samples}"
                                )
                                if sample_idx + 1 == args.num_samples:
                                    json.dump(output, outf)
                                    outf.write("\n")
                                    ...
                                    # VERIFY
                        print(
                            f"required output achieved or number of restarts ran out at attempt #{restart_idx+1}"
                        )
                        ...
                        if ask_skip != "y":
                            if args.debug:
                                print(
                                    "Skipping this example. the autoregressive output already satisfies all the constraints or there's no constraints"
                                )
                                for b in range(batch_size):
                                    print(
                                        "best prediction for all lengths: ",
                                ...
                                print(
                                    f"{text_id}: Skipping this example. the autoregressive output already satisfies all the constraints or there's no constraints"
                                )
                                if args.output_style == "text":
                                    for b in range(batch_size):
                                        outf.write(
                                    ...
                                    for b in range(
                                        batch_size
                                    ):  # batch size is 1 so this is irrelevant
                                        if sample_idx == 0:
                                            output = {
                                                "prompt": {
                                                    "text": source_text,
...
def sentence_completion(prompt, tokens, lossfn):
    ...
    print(tokens)
    new_tokens = lossfn.generate(
        torch.cat([prompt, torch.LongTensor([tokens]).to(lossfn.device)], dim=1)
    )
    print(new_tokens)
    lossfn.args.max_output_length = lossfn.args.max_output_length - 10
