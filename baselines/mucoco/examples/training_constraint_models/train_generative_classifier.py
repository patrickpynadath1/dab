...
@click.option(
    ...
    required=False,
    type=str,
    help="how to reconcile different embeddings (project/freeze/..",
)
...
@click.option(
    ...
    required=False,
    type=str,
    help="model whose embeddings to use",
)
@click.option(
...
def main(
    ...
    labelid2tok = {
        label_id: tokenizer.encode(label_name, add_special_tokens=False)[0]
    ...
    raw_dataset = DatasetDict(
        {"train": traindataset, "validation": valdataset, "test": testdataset}
    )
    ...
    def process_batch(batch):
        ...
        seq_a = (
            (torch.ones(input_ids.shape[0]) * labelid2tok[0])
            .type_as(input_ids)
            .view(-1, 1)
        )
        seq_b = (
            (torch.ones(input_ids.shape[0]) * labelid2tok[1])
            .type_as(input_ids)
            .view(-1, 1)
        )
        ...
        seq_a = (
            (torch.ones(attention_mask.shape[0])).type_as(attention_mask).view(-1, 1)
        )
        seq_b = (
            (torch.ones(attention_mask.shape[0])).type_as(attention_mask).view(-1, 1)
        )
    ...
    class GeDiTrainer(Trainer):
        ...
        def compute_loss(self, model, inputs, return_outputs=False):
            ...
            # input()
            nll = nll.sum(dim=-1) / shift_target.ne(tokenizer.pad_token_id).float().sum(
                dim=-1
            )
