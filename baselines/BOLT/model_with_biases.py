...
class GPTPromptTuningWithbiasesModelMixin:
    ...
    def soft_forward(
        ...
        return loss, output_ids, gpt_logit, senti_losses, onehot_generates
    ...
    def soft_forward_without_decoding(
        ...
        for i in range(gpt_logit.size(1)):
            ...
            weight = 1 * (100 - i) / 100
            gpt_logit[:, i, :] = (
                gpt_logit[:, i, :] + weight * self.lm_head(self.biases[i])
            ) / (1 + weight)
        ...
        cur_sampled_next_token_onehot = (
            torch.nn.functional.one_hot(output_ids, num_classes=self.config.vocab_size)
            .float()
            .to(input_ids.device)
        )
