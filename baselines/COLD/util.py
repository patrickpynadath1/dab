...
if os.path.isdir("/var/karen"):
    os.environ["TRANSFORMERS_CACHE"] = (
        "/var/karen/workspace/Refinement-Generation/cache"
    )
...
def initialize(model, x, length, temperature, device):
    ...
    for i in range(length):
        ...
        logits_so_far = (
            ...
            else torch.cat((logits_so_far, logits), dim=1)
        )
...
def decode_with_model_topk(
    model, y_logits, topk, x_onehot, x_past, tokenizer, extra_mask=None, keywords=None
):
    ...
    for i in range(length):
        ...
        if keywords is not None:
            indices_t = torch.concatenate((indices_t, keywords[:, None, :]), dim=-1)
...
def additional_nll(logits, cur_text_ids):
    return torch.nn.CrossEntropyLoss()(
        logits.view(-1, logits.shape[-1]), cur_text_ids.view(-1)
    )
...
def constraint_loss(logits, cs_onehot, cs_ids):
    ...
    mask = (
        (log_ps_max_ids.unsqueeze(1) == cs_ids_repeat).type(torch.FloatTensor).sum(-1)
    )  # shape: [batch_size, num_cs]
...
def constraint_loss_with_variants(logits, cs_onehot_all, cs_ids_all):
    ...
    for i in range(num_cs):
        ...
        mask = (
            (log_ps_max_ids.unsqueeze(1) == cs_ids_repeat)
        ...
        loss_i = -(
            constraint_max_log_ps_.max(1)[0] * mask
        ).mean()  # average over batch_size
...
def constraint_loss_with_variants_by_ppl(logits, cs_onehot_all, cs_ids_all, probs_t):
    ...
    for i in range(num_cs):
        ...
        cs_onehot_ = (
            cs_onehot_.repeat(batch_size, 1, 1, 1).type(torch.FloatTensor).to(device)
        )
        ppl_max_idx = (ps_t * cs_onehot_).argmax(
            1
        ...
        constraint_max_log_ps_ = (
            (log_ps * ppl_max_idx_onehot).sum(1).sum(-1)
        )  # shape: [batch_size, num_variants]
...
def constraint_loss_all(logits, cs_onehot, cs_ids):
    ...
    log_ps = logits.log_softmax(-1).unsqueeze(2)
    constraint_max_log_ps_ = (
        (log_ps * cs_onehot.unsqueeze(1)).mean(1).sum(-1)
    )  # shape: [batch_size, num_cs]
...
def _constraint_loss2(logits, cs_onehot):
    ...
    max_logits_per_constraint, _ = selected_logits.max(
        0
    )  # select the highest logits for each constraint
...
def has_repeat(sents_for_substr, sents_for_sent):
    ...
    for h in sents_for_substr:
        has_repeat_substring = (
            has_repeat_substring
            or _has_repeat_substring(h)
            or _has_repeat_substring(h, MINLEN=20, MINCNT=2)
        )
...
def write_json_lines(json_lines, fout, model, tokenizer, device):
    with open(fout, "w") as fw:
        for line in json_lines:
            ...
            pg_dict, gens_ranked, pg_dict_top, gens_ranked_top = (
                process_batching_counterfactual_outputs(
...
def compute_loss(
    ...
    c_loss = batch_log_bleulosscnn_ae(
        decoder_outputs=y_logits_.transpose(0, 1), target_idx=z_t, ngram_list=[2, 3]
    )
...
def rank_and_filter(
    candidates, input_text, z, model, tokenizer, device, no_loss_rerank
):
    ...
    for line in candidates:
        ...
        if (
            has_repeat(sents_for_substr=[y], sents_for_sent=x_sents + [y])
            or len(tokenizer.encode(y)) <= 4
        ):
            ...
            loss = compute_loss(
                model, tokenizer, device, x=input_text, z=". " + z, y=". " + y
            )
    ...
    pg_dict = []
    for p, py, l, g in zip(
        ppls_reorder, ppls_y_reorder, loss_reorder, gens_complete_reorder
    ):
