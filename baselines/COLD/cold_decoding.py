...
def decode(
    ...
    if x_t.shape[1] == 1:
        ...
        print(len(x_model_past[0]))
        x_model_past = [(_[0].detach(), _[1].detach()) for _ in x_model_past]
    ...
    texts_to_clean = [None for _ in range(args.batch_size)]
...
def kw(model, tokenizer, device, args, model_back=None):
    prompts = open("prompts/sentiment_prompts_15.txt", "r").readlines()
