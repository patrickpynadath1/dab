...
class Switching_GPTJModel(nn.Module):
    ...
    def generate_multiple(
        self, prompts, steer_values, min_length=20, max_length=100, seed=None
    ):
        """
        prompt: a string
        steer_values
        ...
        self.steer.set_value(steer_values)
        with torch.no_grad():
            input_ids = self.tokenizer(prompts, return_tensors="pt").input_ids.to(
                self.device
            )
            gen_tokens = self.model.generate(
                input_ids,
    ...
    def evidence_words(
        ...
        input_ids = torch.LongTensor(tokenized["input_ids"]).to(self.device)
        input_ids = input_ids.expand(2, -1)
        attention_mask = torch.LongTensor(tokenized["attention_mask"]).to(self.device)
        attention_mask = attention_mask.expand(2, -1)
        steer_values = torch.zeros(2, self.num_steers).to(self.device)
        steer_values[0] = original_steer_values
        steer_values[1] = (-original_steer_values > 0) * 2 - 1
        ...
        evidence_segments = find_max_subspans(
            token_evidence.cpu().numpy().tolist(), max_segments, max_length
        )[0]
        evidence_segments = [(_seg[0] + 1, _seg[1] + 1) for _seg in evidence_segments]
        start = 0
        output = []
        color = (
            "gray"
            if original_steer_values.shape[0] > 1
            else "red" if original_steer_values[0] > 0 else "blue"
        )
        if len(evidence_segments) > 0:
            for _segment in evidence_segments:
                if _segment[0] > start:
                    output.append(
                        (self.tokenizer.decode(tokens[start : _segment[0]]), None)
                    )
                output.append(
                    (self.tokenizer.decode(tokens[_segment[0] : _segment[1]]), color)
                )
                start = _segment[1]
            length = tokens.shape[-1]
            if _segment[1] < length:
                output.append(
                    (self.tokenizer.decode(tokens[_segment[1] : length]), None)
                )
        else:
            output = [(prompt, None)]
    ...
    def steer_analysis(
        ...
        best_guess_value = (
            min_value + (max_value - min_value) / (bins - 1) * best_guess.item()
        )

        token_evidence = self.evidence_words(
            prompt,
            steer_values[best_guess],
        )
