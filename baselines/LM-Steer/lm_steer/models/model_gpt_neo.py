...
class Switching_GPTNeoModel(nn.Module):
    ...
    def evidence_words(
        ...
        if isinstance(comparing_steer_values, list):
            comparing_steer_values = torch.Tensor(comparing_steer_values).to(
                self.device
            )
        if (comparing_steer_values[0] - comparing_steer_values[1]).abs().sum() <= 0.2:
            return [(prompt, None)]
    ...
    def steer_analysis(self, prompt, steer_dim, min_value=-3, max_value=3, bins=7):
            ...
            for _punc in punctuations:
                if token_i == n_tokens or _punc in span:
                    new_span = self.tokenizer.decode(
                        input_ids[0][start : token_i - 1]
                    ).strip()
                    if len(new_span) <= 1:
                        break
                    word_evidence_list.append(
                        (new_span, np.array(token_evidence[start : token_i - 1]).mean())
                    )
                    start = token_i - 1
                    break
