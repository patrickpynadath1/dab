...
def compute_bertscore_prompt(prompt_gens, 
                             ref_gens, mode='mean', metric='f1'): 
    ...
    for cur_prompt in prompt_gens:
        ...
        results = bertscore.compute(predictions=single_prompt, references=ref_gens, lang='en')
...
def load_ref_texts(base_dir,
                   cur_prompt_idx, 
                   cur_prompt_text, 
                   topic):
    refs = []
    for kw in topic_kw_dct[topic]:
        ref_gens = open(f"{base_dir}/{topic}/{kw}_{cur_prompt_idx}.txt", "r").readlines()
        for r in ref_gens:
            refs.append(r)
    return refs 
